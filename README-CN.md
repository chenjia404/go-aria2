...
与经典 `aria2` 守护进程相比，`go-aria2` 侧重提供：

- **BitTorrent v2** — 在 v1 之外支持现代 v2 种子（混合 / 纯 v2）；不少 aria2 使用场景仍以 v1 为主。
- **ED2K（eDonkey2000）** — 内置 ED2K，走真实驱动层；aria2 不包含该协议。
- **不人为限制“线程”规模** — 并发以 goroutine 与 Go 调度为主，不像许多原生下载守护进程那样受固定工作线程池上限约束。
- **持续维护** — 项目仍在积极迭代，依赖与功能会长期更新，更适合长期部署。

以上是对 aria2 的补充而非否定其长处；差距与边界见 [已知限制](#已知限制)。
...
- `bt-request-peer-speed-limit`：整 torrent 下载速度低于该阈值（默认 50K）或尚无 peer 时触发 DHT announce 增连；做种时在未达 `bt-max-peers` 且上传未饱和时同样增连
- `getGlobalOption` 补齐 `enable-dht`、`listen-port`、`bt-max-peers` 等启动期选项的只读快照
